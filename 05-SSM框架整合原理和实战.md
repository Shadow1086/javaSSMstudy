...
> 本文聚焦 SSM（Spring + SpringMVC + MyBatis）整合的核心问题：容器划分、组件归属、调用链路、配置拆分与工程实践。
>
> 阅读目标是形成可落地的整合心智模型，做到**知道每个组件放哪、为什么这么放、出现问题怎么查**。
...
本质上，整合就是将三层架构和框架核心 API 组件交给 SpringIoC 容器管理。

一个容器可能就够了，但常见操作是创建 **两个IoC容器**（web容器和root容器），组件分类管理：

1. **分离关注点**：各层次组件聚焦各自责任和功能
2. **解耦合**：各层次组件分离装配不同容器，模块可独立操作和测试
3. **灵活配置**：每个容器有各自配置，配置文件更清晰

#### 1.2.2 第二问：每个 IoC 容器对应哪些类型组件？

| 容器名 | 盛放组件 | 对应配置类 |
| --- | --- | --- |
| **web 容器**（子容器） | Controller、SpringMVC 核心组件（HandlerMapping、HandlerAdapter 等） | `WebMvcConfig` |
| **root 容器**（父容器） | Service、AOP、TX、DataSource、MyBatis、Mapper 等 | `ServiceConfig`、`MapperConfig` |
...
**结论**：web容器是root容器的**子容器**，父子容器关系。

- **父容器（root容器）**：盛放 service、mapper、mybatis 等相关组件
- **子容器（web容器）**：盛放 controller、web 相关组件
...
> **调用方向**：Controller（web 容器）→ Service（root 容器）→ Mapper（root 容器）。子容器可以访问父容器的 Bean，反之不行。

#### 1.2.4 第四问：具体多少配置类以及对应容器关系？

配置类的数量不是固定的，但至少要两个。建议三层架构每层对应一个配置类：
...
3. mybatis和持久层配置类
    ...
    位置：MapperJavaConfig.java(命名随意)Java
@Configuration
...
4. 拆分配置
    ...
    数据库配置类（DataSourceJavaConfig.java）Java
@Configuration
@PropertySource("classpath:jdbc.properties")
...
mybatis配置类（MapperJavaConfig.java）Java
@Configuration
public class MapperJavaConfig {
...
4. mapper
    ...
    mapper接口  包：com.atguigu.mapper Java
public interface EmployeeMapper {

     List<Employee> queryAll();
}

mapper XML 文件位置： resources/mappersXML
<?xml version="1.0" encoding="UTF-8" ?>
<!DOCTYPE mapper
...
2. 更新npm版本
    ...
    > node16.16.0对应的npm版本过低！需要升级！Java
