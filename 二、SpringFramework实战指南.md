...
-   **3.1.1 什么是组件?**
    ...
    ![](assets/image_Gusp1HuW3X.png)

    整个项目就是由各种组件搭建而成的：

    ![](assets/image_evy1Qf9TmX.png)
-   **3.1.2 我们的期待**
    -   有人替我们创建组件的对象
    -   有人帮我们保存组件的对象
    -   有人帮助我们自动组装
...
配置类：
 	...
 	1. 配置类的作用：完全注解开发，用来替代xml文件。
 	 	1. 替代扫描标签->注解
 	 	2. 引用外部配置标签 -> 注解
 	 	3. 替代<bean 标签 -> 注解
 	2. 注解：
 	 	1. 如何声明配置类：@Configuration
 	 	2. 如何扫描包：@ComponentScan
 	 	3. 如何引用外部配置：@PropertySource
 	 	4. 如何声明bean :@Bean+方法
 	1. 使用配置类对应的ioc容器变化为:AnnotationConfigApplicationContext
...
1.  所有内容写到`xml格式配置文件`中
2.  声明bean通过`\<bean`标签
3.  \<bean标签包含基本信息`（id,class）`和属性信息 `\<property name value / ref`
4.  引入外部的`properties文件`可以通过`\<context:property-placeholder`
...
1.  **代理模式**
    ...
    <img src="https://cdn.jsdelivr.net/gh/Shadow1086/myPicture@master/uPic/2026/03/14/20-38-mzLkzQ" style="zoom:50%;" />

    有代理场景：

    <img src="https://cdn.jsdelivr.net/gh/Shadow1086/myPicture@master/uPic/2026/03/14/20-39-yseC1Q" style="zoom:50%;" />
...
3.  **动态代理**
    ...
    -   cglib：通过继承被代理的目标类实现代理，所以不需要目标类实现接口！（认干爹）
        JDK动态代理技术实现（了解）
        ![](https://cdn.jsdelivr.net/gh/Shadow1086/myPicture@master/uPic/2026/03/14/20-40-s13IPE)
...
1.  **JoinPoint接口**
    ...
    | 方法                  | 返回类型              | 典型用途                                   |
    | --------------------- | --------------------- | ------------------------------------------ |
    | `getArgs()`           | `Object[]`            | 获取目标方法参数（做参数日志、校验）       |
    | `getSignature()`      | `Signature`           | 获取方法签名信息（方法名、修饰符、声明类型） |
    | `getTarget()`         | `Object`              | 获取被代理的目标对象（原对象）             |
    | `getThis()`           | `Object`              | 获取当前代理对象（AOP 代理）               |
    | `getKind()`           | `String`              | 连接点类型（Spring AOP 里通常是方法执行）  |
    | `getSourceLocation()` | `SourceLocation`      | 源码位置信息（调试场景用）                 |
    | `getStaticPart()`     | `JoinPoint.StaticPart`| 连接点静态部分（较少直接用）               |
    | `toShortString()`     | `String`              | 简短描述，适合日志                         |
    | `toLongString()`      | `String`              | 完整描述，排查问题更详细                   |
    | `toString()`          | `String`              | 普通描述                                   |
...
2.  **切点表达式语法**
    ...
    ![](https://cdn.jsdelivr.net/gh/Shadow1086/myPicture@master/uPic/2026/03/14/20-38-87PVk2)
...
<img src="https://cdn.jsdelivr.net/gh/Shadow1086/myPicture@master/uPic/2026/03/14/20-36-Q4nKbN" style="zoom:50%;" />

实际意义

实际开发时，如果有多个切面嵌套的情况，要慎重考虑。例如：如果事务切面优先级高，那么在缓存中命中数据的情况下，事务切面的操作都浪费了。

<img src="https://cdn.jsdelivr.net/gh/Shadow1086/myPicture@master/uPic/2026/03/14/20-37-oGTqmK" style="zoom:50%;" />
...
![](https://cdn.jsdelivr.net/gh/Shadow1086/myPicture@master/uPic/2026/03/14/20-35-HkgFup) 
...
1.  准备工作
    ...
    4. 把测试基于注解功 能时的Java类复制到新module中，去除所有注解。
2.  配置Spring配置文件
    ```xml
    <!-- 配置目标类的bean -->
...
![](assets/image_EbfR90kP01.png)

对没实现接口的类应用切面new

![](assets/image_wUfCJhIWI3.png)
